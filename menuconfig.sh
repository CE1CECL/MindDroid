...
make -C "$(pwd)/am1808/DaVinci-PSP-SDK-03.20.00.13/src/kernel/linux-03.20.00.13/" -j$(nproc --all) ARCH="arm" CROSS_COMPILE="$(pwd)/CodeSourcery/Sourcery_G++_Lite/bin/arm-none-linux-gnueabi-" INSTALL_MOD_PATH="$(pwd)/system/" menuconfig
cp -rfv "$(pwd)/am1808/DaVinci-PSP-SDK-03.20.00.13/src/kernel/linux-03.20.00.13/.config" "$(pwd)/config"
